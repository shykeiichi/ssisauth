...
public class CommandHelpop implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if(args.length < 1) {
            sender.sendMessage("/helpop <meddelande>");
            return true;
        }
        ...
        request = HttpRequest.newBuilder()
                .uri(URI.create(apiip + "/api/v1/helpop/add?username=" + player.getName() + "&message=" + String.join("+", args) + "&class=" + getClassOfPlayer(player)))
                .build();
